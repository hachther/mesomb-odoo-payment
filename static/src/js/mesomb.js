odoo.define('payment_mesomb.mesomb', function (require) {
"use strict";
...
ajax.loadXML('/payment_mesomb/static/src/xml/mesomb_templates.xml', qweb);
...
var observer = new MutationObserver(function (mutations, observer) {
    for (var i = 0; i < mutations.length; ++i) {
        for (var j = 0; j < mutations[i].addedNodes.length; ++j) {
            if (mutations[i].addedNodes[j].tagName.toLowerCase() === "form" && mutations[i].addedNodes[j].getAttribute('provider') === 'mesomb') {
...
function displayError(message) {
    var wizard = $(qweb.render('mesomb.error', {'msg': message || _t('Payment error')}));
...
function _redirectToStripeCheckout(providerForm) {
    ...
    var mesomb = Stripe(_getStripeInputValue('mesomb_key'));

    mesomb.redirectToCheckout({
...
$.getScript("https://js.mesomb.com/v3/", function (data, textStatus, jqxhr) {
    observer.observe(document.body, {childList: true});
    _redirectToStripeCheckout($('form[provider="mesomb"]'));
});
});